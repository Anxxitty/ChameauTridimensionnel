...
void main() {
    vec3 light_obj = o_light_pos - frag_pos;
    vec3 light_dir = normalize(light_obj);
    ...
    vec3 diffuse = obj_col.xyz * light_col * max(0.0, dot(light_dir, norm));
    ...
    frag_color = vec4((diffuse + ambient_light + specular)*light_intensity/length(light_obj), alpha);
}