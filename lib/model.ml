...
let parseVector4f string bigarray index =
  let splitString = String.split_on_char ' ' (String.trim (String.sub string 1 (String.length string - 1))) in
  let rec fillBigarray l i = match l with
    | [] -> ()
    | a::q -> bigarray.{i} <- float_of_string (String.trim a); fillBigarray q (i+1) in
  if List.length splitString = 3 then bigarray.{(index*4)+3} <- 1.0; (*If the vertex's w coordinate is ommited, default to 1.0*)
  fillBigarray splitString (index*4)
...
let rec parseFaceTriangles stringList faces = match stringList with
    ...
    | a::q -> (
      ...
      match faceInfo with | [] -> [] | v::l -> parseFaceTriangles q ((int_of_string v - 1)::faces)
    )
...
let parseFaceQuads stringList faces =
  ...
  let firstTri = triangle stringList 0 [] 0 in
  let secondTri = triangle stringList 0 [] 2 in
...
let loadObjModel modelPath =
  ...
  let rec parse lineList meshIndex faces = match lineList with
    ...
    | a::q -> (
      ...
      else if String.starts_with ~prefix:"f " a then parse q meshIndex (parseFace (List.rev (String.split_on_char ' ' (String.trim a))) faces) (*face definition line*)
      else parse q meshIndex faces (*unknown line -> ignored*)
    )
  in parse (List.rev modelLinesUnparsed) 0 []
