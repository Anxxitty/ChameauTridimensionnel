...
class ['a, 'b] vertexAttribute ~(attributeType : ('a, 'b) vectorKind) ~numberOfVertices ?(vectorList : 'a vector list option) () =
  ...
  let () = (
    match vectorList with
      | Some vectorListVal -> (
          logger Debug "vertexAttribute: Filling vertexAttribute with data given on construction.";
  ...
  object (self)
    ...
    method setVertexAttribute ~index ~value = 
      let () = logger Debug "vertexAttribute: setting a vertexAttribute" in
      match _dimension with
...
class ['a, 'b] buffer ~bufferType ~(kind : ('a, 'b) kind) =
  object (self)
    ...
    method writeRawData ~(size : int) ~(data : ('a, 'b) Gl.bigarray) ~(usage : int) =
      logger Debug "buffer: Copying data into a buffer through writeRawData.";
      self#bind ();
    ...
    method writeElementBuffer ~(indices : 'a list) ~(usage : int) =
      logger Debug "buffer: Copying data into a buffer through writeElementBuffer.";
    ...
    method writeVertexAttributes ~(vertexAttributes : (('a, 'b) vertexAttribute) list) ~(usage : int) =
      logger Debug "buffer: Copying data into a buffer through writeVertexAttributes.";
      self#bind ();
      match vertexAttributes with
        ...
        | a::q -> (
            ...
            let rec checkDataCoherenceAndDetermineSize sizeAcc l = match l with
              | [] -> sizeAcc
              | a::q ->
                if a#getNumberOfVertices <> numberOfVertices
