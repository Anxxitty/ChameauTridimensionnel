...
let () =
  ...
  let cube_drawable = new drawable ~vertex_array:cube_model#get_vao ~shader_program ~number_of_drawn_vertices:cube_model#get_number_of_drawn_vertices ~scene_coordinates:{x=0.0;y=3.0;z=0.0} ~local_rotation:identity_matrix4f ~scale:{x=1.5;y=1.5;z=1.5} in
  let cat_initial_rot = rotation_matrix4f ~axis:{x=1.0;y=0.0;z=0.0} ~angle:(rad_of_deg (-90.0)) in
  ...
  let tie_drawable = new drawable ~vertex_array:tie_model#get_vao ~shader_program ~number_of_drawn_vertices:tie_model#get_number_of_drawn_vertices ~scene_coordinates:{x=0.0;y=3.0;z=0.0} ~local_rotation:identity_matrix4f ~scale:{x=0.1;y=0.1;z=0.1} in
  let dark_vador_tie_drawable = new drawable ~vertex_array:dark_vador_tie_model#get_vao ~shader_program ~number_of_drawn_vertices:dark_vador_tie_model#get_number_of_drawn_vertices ~scene_coordinates:{x=12.0;y=3.0;z=0.0} ~local_rotation:identity_matrix4f ~scale:{x=0.05;y=0.05;z=0.05} in
  let hexagon_drawable = new drawable ~vertex_array:hexagon_model#get_vao ~shader_program ~number_of_drawn_vertices:hexagon_model#get_number_of_drawn_vertices ~scene_coordinates:{x=0.0;y=3.0;z=(-20.0)} ~local_rotation:identity_matrix4f ~scale:{x=0.5;y=0.5;z=0.5} in
  let table_drawable = new drawable ~vertex_array:table_model#get_vao ~shader_program ~number_of_drawn_vertices:table_model#get_number_of_drawn_vertices ~scene_coordinates:{x=0.0;y=(-16.0);z=0.0} ~local_rotation:identity_matrix4f ~scale:{x=0.25;y=0.25;z=0.25} in
  let torus_drawable = new drawable ~vertex_array:torus_model#get_vao ~shader_program ~number_of_drawn_vertices:torus_model#get_number_of_drawn_vertices ~scene_coordinates:{x=0.0;y=3.0;z=12.0} ~local_rotation:identity_matrix4f ~scale:{x=3.0;y=3.0;z=3.0} in
  ...
  let render aspect_ratio () =
    ...
    cube_drawable#set_rotation (rotation_matrix4f ~axis:{x=0.0;y=1.0;z=0.0} ~angle:(sin(GLFW.getTime ())));
    cat_drawable#rotate ~axis:{x=0.0;y=0.0;z=1.0} ~angle:0.01;
